...
| Category                      | Script                                                     | OS                  | Language   | Description                                                      |
|-------------------------------|------------------------------------------------------------|---------------------|------------|------------------------------------------------------------------|
| Network Discovery & Auditing  | **[network_scanner.py](#network-inventory-scanner)**       | Linux/macOS         | Python     | Subnet scanner with ARP & Nmap integration.                      |
| Network Discovery & Auditing  | **[proxmox_net_check.sh](#proxmox-lxc-network-auditor)**   | Proxmox             | Bash       | Rapidly audit network interfaces for LXC containers.             |
| Performance & Connectivity    | **[check_latency.py](#high-precision-latency-tester)**     | Linux/macOS         | Python     | Detailed latency and packet loss analysis tool.                  |
| Performance & Connectivity    | **[check_latency_win.ps1](#windows-latency-tester)**       | Windows             | PowerShell | Windows-native latency monitor with reports.                     |
...
`services/` holds the guides for this pipeline; `provisioning/` holds each service's actual OpenTofu and Ansible files, one directory per service, kept separate from the guides for findability. Start with [services/opentofu_ansible_setup_guide.md](services/opentofu_ansible_setup_guide.md) (one-time runner setup) and [services/lxc_instance_provisioning_guide.md](services/lxc_instance_provisioning_guide.md) (the provisioning pattern, with [provisioning/vault/](provisioning/vault/) as the real, deployed worked example and [provisioning/baseline_image/](provisioning/baseline_image/) as the generic, undeployed template to copy for a new service). [services/gitops_setup_guide.md](services/gitops_setup_guide.md) covers automating that pipeline with Atlantis, so a merged PR applies changes with no manual `git pull`/`tofu apply`. [services/deploying_a_new_service.md](services/deploying_a_new_service.md) ties all three together into the actual end-to-end path, from an empty service directory to a running, GitOps-managed service.
