...
## 5. atlantis.yaml: binding a project directory to a workflow

Each service gets its own directory under `provisioning/<service>/`, a standalone, copyable `main.tf` rather than a shared Terraform module (see [lxc_instance_provisioning_guide.md](lxc_instance_provisioning_guide.md) for the pattern and why).
