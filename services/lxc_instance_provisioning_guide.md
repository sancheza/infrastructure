...
|---|---|---|
| `main.tf` | Yes | Resource definitions — see [vault-provision/main.tf](vault-provision/main.tf) |
| `inventory.ini.tpl` | Yes | Ansible inventory template |
| `ansible.cfg` | Yes | Ansible defaults |
| `deploy_<service>.yml` | Yes | Service-specific playbook |
| `run.sh` | Yes | Docker wrapper, pre-pointed at this workspace |
| `terraform.tfvars.example` | Yes | Placeholder values — copy to `terraform.tfvars` |
| `terraform.tfvars` | **No** (gitignored) | Real credentials — created once, by hand, on the runner |
...
The `admin_ssh_key` task uses `ansible.builtin.lineinfile`, part of `ansible-core`, rather than `ansible.posix.authorized_key` — the runner image only installs `ansible-core` (Section 2 of the setup guide), and `ansible.posix` is not a bundled collection.
